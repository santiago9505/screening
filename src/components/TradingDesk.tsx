...
export default function TradingDesk() {
  ...
  return <section className="trading-desk" aria-label="Seguimiento de acciones">
    ...
    <nav className="desk-tabs" aria-label="Listas de seguimiento">{(Object.keys(descriptions) as DeskBucket[]).map(key=><button key={key} onClick={()=>{setBucket(key);setSelected(null);setQuery('');}} className={bucket===key?'active':''}>{names[key]}<span>{rows.filter(r=>r.bucket===key).length}</span></button>)}</nav>
